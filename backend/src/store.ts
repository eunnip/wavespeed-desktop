...
function mapLegacyStore(data: StoreData): StoreData {
  ...
  data.uploads = data.uploads.map((record) => ({
    ...
    fileUrl: record.fileUrl ?? (record as { file_url?: string }).file_url ?? undefined,
  }));
  ...
  data.jobs = data.jobs.map((job) => ({
    ...
    parameters:
      job.parameters && typeof job.parameters === "object" && !Array.isArray(job.parameters)
        ...
        : {},
    providerTaskId:
      job.providerTaskId ?? (job as { provider_task_id?: string }).provider_task_id ?? undefined,
    providerResultUrl:
      job.providerResultUrl ??
      (job as { provider_result_url?: string }).provider_result_url ??
      undefined,
    outputs: (job.outputs ?? []).map((output) => ({
      ...output,
      storageKey: output.storageKey ?? (output as { relativePath?: string }).relativePath ?? "",
      sourceUrl:
        output.sourceUrl ?? (output as { source_url?: string }).source_url ?? undefined,
    })),
  }));
...
function mapJobRow(row: SupabaseJobRow): JobRecord {
  return {
    ...
    providerResultUrl: row.provider_result_url ?? undefined,
    status: row.status,
    createdAt: row.created_at,
    updatedAt: row.updated_at,
    ...
    outputs: (row.outputs ?? []).map((output) => ({
      ...
      sourceUrl: output.source_url ?? undefined,
    })),
  };
}
...
function jobToRow(job: JobRecord): SupabaseJobRow {
  return {
    ...
    provider_result_url: job.providerResultUrl ?? null,
    status: job.status,
    ...
    outputs: job.outputs.map((output) => ({
      ...
      source_url: output.sourceUrl ?? null,
    })),
  };
}
